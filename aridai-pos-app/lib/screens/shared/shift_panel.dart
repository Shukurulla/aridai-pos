...
/// Reusable register-shift panel — open / close the active shift and review the
/// history of closed shifts. Used by both the branch-admin "Смена" tab and the
/// cashier home's "Смена" segment so the behaviour stays identical (including
/// the auto-suggested closing cash = opening + cash revenue).
///
/// It manages its own data; [onActiveChanged] fires after every load with
/// whether a shift is currently open, so the host can reflect it (e.g. a
/// header subtitle).
class ShiftPanel extends StatefulWidget {
  const ShiftPanel({super.key, this.onActiveChanged, this.padding});

  final ValueChanged<bool>? onActiveChanged;
  final EdgeInsetsGeometry? padding;

  @override
  State<ShiftPanel> createState() => ShiftPanelState();
}

class ShiftPanelState extends State<ShiftPanel> {
  ...
  /// Public so a host can trigger a refresh (e.g. a shared refresh button).
  Future<void> load() async {
    ...
    try {
      ...
      widget.onActiveChanged?.call(_active != null);
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        padding: widget.padding ?? const EdgeInsets.fromLTRB(20, 16, 20, 28),
        children: [
          if (active != null)
            _ActiveCard(
              shift: active,
          ...
          if (closed.isEmpty)
            Container(
              ...
              child: Text('Закрытых смен пока нет',
      ...
      child: Stack(
        children: [
          Positioned(
            right: -12,
            top: -12,
            child: Diamond(size: 96, color: AppColors.ok.withValues(alpha: 0.18)),
          ),
          Padding(
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          ...
          style: numStyle(size: 14, weight: FontWeight.w500, color: Colors.white),
        ),
      ],
    );
  }
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(
            'Смена не открыта',
            style: sansStyle(size: 15, weight: FontWeight.w600, color: AppColors.ink),
          ),
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Text(
                shift.shiftNumber != null ? 'Смена №${shift.shiftNumber}' : 'Смена',
                style: sansStyle(size: 14, weight: FontWeight.w600, color: AppColors.ink),
              ),
              const Spacer(),
              Text(
                fmtMoney(shift.revenue),
                style: numStyle(size: 15, weight: FontWeight.w600, color: AppColors.ink),
              ),
            ],
          ),
          ...
          Wrap(
            ...
            runSpacing: 6,
            children: [
              _Pill(label: '${fmtNumber(shift.ordersCount)} заказов'),
              if (hasDisc)
                _Pill(label: 'Расхожд. ${fmtMoney(disc)}', danger: true)
              else
